...
def patch_notebook():
    ...
    # 1. Extract Laplace functions from Cell 3
    ...
    if "compute_uncertainties_core" not in pipeline_source_text:
        ...
        # Integrate Stage 2 Laplace (Element Loop)
    ...
    if "compute_uncertainties_joint" not in pipeline_source_text:
        ...
        # Fix dictionary keys
        pipeline_source_text = pipeline_source_text.replace(
            "'inferred_errors': [], 'inferred_errors': [], 'inferred_errors': [],",
            "'inferred_errors': [],"
        )

    # 5. Insert Laplace functions at the top of Cell 1
    new_cell_1_source = laplace_source + ["\n"] + [l + "\n" for l in pipeline_source_text.split("\n")]
    nb['cells'][1]['source'] = new_cell_1_source

    # 6. Delete old Cell 3
    del nb['cells'][3]

    # 7. Update Reporting in current Cell 4 (was Cell 5)
    # Actually, Cell Indices shifted. Cell 3 is now Markdown, Cell 4 is Execution, Cell 5 is Plotting.
    # We want to call report_map_results_enriched(results) in the plotting part.
    for cell in nb['cells']:
        src = "".join(cell.get('source', []))
        if "report_map_results(" in src and "def " not in src:
            cell['source'] = [l.replace("report_map_results(", "report_map_results_enriched(") for l in cell['source']]

    with open(notebook_path, 'w', encoding='utf-8') as f:
        json.dump(nb, f, indent=1, ensure_ascii=False)
    print("Notebook perfectly integrated.")
